...
var game = new function () {
    ...
    var designFile = "design.htm";
    var questionsFile = "questions.htm";
    var soundsFile = "sounds.htm";

    if (!empty(GAMEPREFIX)) {
        soundsFile = GAMEPREFIX + "-sounds.htm";
        questionsFile = GAMEPREFIX + "-questions.htm";
        designFile = GAMEPREFIX + "-design.htm";
    }
    ...
    function finishGame(win) {
        ...
        try {
			...
			// 1º Obtener los valores anteriores del SUSPENDATA
			//score data  completed
			
			var num_intento = 0
			var score_ant= SCOGetValue("score")
			
			var status_ant= SCOGetValue("completed")
			var data_ant= SCOGetValue("data")
			if ((data_ant!="")&&(data_ant!=null))
			{
				var num_intento=parseInt(data_ant.split("Intento: ")[1].split(" ")[0])
				var punt_ant=parseInt(data_ant.split("Puntuacion: ")[1].split(" ")[0])
				var aciertos_ant=parseInt(data_ant.split(" Aciertos: ")[1].split(" ")[0])
			}
			else
			{
			...
			alert ("["+ (num_intento+1)+"] Valores Anteriores  score_ant:[" + score_ant  + "] punt_ant:[" + punt_ant + "] aciertos_ant=["+aciertos_ant+"] status_ant:[" + status_ant +"]")
			
			// 2º Calcular los nuevos basandonos en los anteriores
		
		
			//puntuacionBase20= Math.round(correctAnsweredCount * 20/questionCount)
			puntuacionBase20= correctAnsweredCount * 10   // nos da una nota de 0 a 100
			if (num_intento==0)
			{
				strResultado="Intento: " + (num_intento+1)  + " Puntuacion: " + puntuacionBase20 + "  Aciertos: " + correctAnsweredCount + " de " +  questionCount 			
			}
			else
			{
				// Calculamos promedio acumulado
				//var nuevapuntuacion= (((punt_ant * num_intento /10) + correctAnsweredCount)/ (num_intento+1) )*10
				var nuevapuntuacion= (correctAnsweredCount + aciertos_ant)/ (num_intento+1) *10
				strResultado="Intento: " + (num_intento+1)  + " Puntuacion: " + nuevapuntuacion + "  Aciertos: " + (correctAnsweredCount + aciertos_ant ) + " de " +  questionCount 			
			}
			//strResultado="intentado"
			//alert(strResultado)
			
			
		
            SCOSetValue("time", gameTime);
            //SCOSetValue("score", 100); MODIFICADO AJPG PARA QUE ALMACENE PORCENTAJE AVANCE EN BASE 20
			//SCOSetValue("score", puntuacionBase20 );
			SCOSetValue("score", nuevapuntuacion);
			
			if(puntuacionBase20 >= 50)
			{
				SCOSetValue("passed", 1);
			}	
			else
			{
				SCOSetValue("failed", 1);
			}
			 SCOSetValue("data", strResultado);
			 alert("Valores actuales " + strResultado)
					
            SCOCommit();
        } catch (e) {
			//alert("Scorm failed -"+ e)
